...
void time_one_step_cd(int nx, int nz, float dt, float dx, float dz, int np, double *d,
                      float **v, float **pp, float **pm, float **p);

extern void fdcoeff_2nd_regulargrd(float r, int n, double *cc);
...
//    FILE *fp_seis = stdout;
    ...
    if (!getparstring("file_snap", &file_snap))	/// \param[in] file_snap filename of snapshot, default file_snap = "snapfd", no extension.
    	file_snap = "snapfd";
//        err("must specify file_snap!\n");
    if (!getparstring("file_vseis", &file_vseis))	/// \param[in] file_vseis filename of velocity model, default file_vseis = "vseisfd.su"
    	file_vseis = "vseisfd.su";
//        err("must specify file_vseis!\n");
    if (!getparstring("file_hseis", &file_hseis))	/// \param[in] file_hseis filename of velocity model, default file_vseis = "hseisfd.su"
    	file_hseis = "hseisfd.su";
...
//        np = 9;
    ...
    fdcoeff_2nd_regulargrd(0, norder, d);
